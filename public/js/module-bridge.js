...
(function () {
  ...
  function revokePendingToken() {
    ...
    /**
     * Revoke unexchanged token with MODULE backend.
     * 
     * Called when iframe unloads before token exchange completes.
     * Prevents token replay if another actor obtains the token string.
     */
    var moduleOrigin = window.location.origin;
    
    fetch(moduleOrigin + "/api/sso/revoke", {
      method: "POST",
      credentials: "include",
      keepalive: true,
      headers: {
  ...
  function exchangeToken(token) {
    /**
     * Exchange token with MODULE backend (not portal).
     * 
     * The token is issued by portal but exchanged with module.
     * Module validates portal signature and creates session.
     */
    var moduleOrigin = window.location.origin;
    
    return fetch(moduleOrigin + "/api/sso/exchange", {
      method: "POST",
      credentials: "include",
      headers: {
        "Content-Type": "application/json",
